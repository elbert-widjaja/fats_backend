...
const apiServer = s.getAttribute("data-apiServer") || window.location.origin + '/api/image/'; //the full TensorFlow Object Detection API server url
...
function drawBoxes(objects) {
    ...
    array_objects.detections.filter(object => object.class_name).forEach(object => {
        ...
        if (mirror) {
            object.x = drawCanvas.width - (object.x + object.width)
        }
        
        drawCtx.fillText(object.class_name + " - " + Math.round(object.score * 100) + "%", x + 5, y + 20);
        drawCtx.strokeRect(object.x, object.y, object.width, object.height);
